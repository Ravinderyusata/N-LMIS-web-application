...
#store_code_label,#store_name_label,#store_types_label,#default_ord_store_label,#country_name_label,
#state_name_label,#start_date_label,#mtp_label{
font-weight: bold;
}
</style>
</head>
<body style="margin: 0px;" onload="setRole()">
	...
	<div class="button_bar" id="button_bar">
		<ul>
			<li><a id="addBtn" class="w3-btn w3-ripple" onclick="addLgaStore()"> <img alt="add"
					src="resources/images/file_add.png">Add
			</a></li>
			<li><a id="editBtn" class="w3-btn w3-ripple" onclick="editLgaStore(this.id)"> <img alt="edit"
					src="resources/images/file_edit.png">Edit
			</a></li>
			<li><a id="overViewBtn" class="w3-btn w3-ripple" onclick="editLgaStore(this.id)"> <img alt="overViewBtn"
			...
			<li><a class="w3-btn w3-ripple" href="lga_store_list_export"> <img alt="export"
					src="resources/images/Export_load_upload.png">Export
			</a></li>

			<li><a class="w3-btn w3-ripple" onclick="refreshLgaStoreList()"> <img alt="refresh"
...
<!-- toolbar for table -->
    <div id="table_toolbar" style="padding:2px 5px;">
        Store Type:   <select id="storetype_combobox" class="easyui-combobox"  style="width:150px">
        </select>
        Store Name:   <select id="storename_combobox" class="easyui-combobox"  style="width:200px">
        </select>
	...
	<table id="lgaSoreListTable" class="easyui-datagrid"
	 ...
	 <div id="form_dialog" class="easyui-dialog" style="width:430px;height:480px;padding:10px 20px"
            ...
            <td>
	            <div id="store_code_div">
	                    ...
	                    <f:input  id="store_code_textbox"  class="easyui-textbox" path="x_WAREHOUSE_CODE"/>
	              </div>
            </td>
             <td>
             <div class="store_name_div">
                    <label id="store_name_label">*Store Name:</label>
                    <f:input id="store_name_textbox"  class="easyui-textbox" path="x_WAREHOUSE_NAME"/>
                </div>
             </td>
            </tr>
             <tr>
            <td>
	            <div class="description_div">
                    ...
                    <f:textarea id="description_textbox"  class="easyui-textbox" path="x_WAREHOUSE_DESCRIPTION"/>
                </div>
            </td>
             <td>
                ...
                <div class="store_types_div">
                    ...
                    <f:select  id="store_type_combobox_form"  class="easyui-combobox" cssStyle="width:120px;" path="" />
                    <input id="store_type_combobox_form_field" type="hidden" name="x_WAREHOUSE_TYPE_ID">
                </div>
             </td>
            </tr>
              <tr>
	            <td>
		            <div class="address_div">
	                    ...
	                    <f:input id="address_textbox"  class="easyui-textbox" path="x_ADDRESS1"/>
	                </div>
	            </td>
	             <td>
		            <div id="default_ord_store_div">
		             <label id="default_ord_store_label">*Default Ordering Store:</label>
	                    <f:select id="default_ord_store_combobox_form"  class="easyui-combobox" cssStyle="width:150px;" path=""/>
	                    <input id="default_ord_store_combobox_form_field" type="hidden" name="x_DEFAULT_ORDERING_WAREHOUSE_ID">
	                </div>
               </td>
            </tr>
            <tr>
	              ...
	              <td>
		            <div id="state_name_div">
		             <label id="state_name_label">*State Name:</label>
	                    <f:select id="state_name_combobox_form"  class="easyui-combobox" cssStyle="width:150px;" path="x_STATE_ID"/>
	                </div>
               </td>
            </tr>
            <tr>
            <td>
		            <div class="phone_no_div">
	                    <label id="phone_no_label" class="label-top">Telephone No:</label>
            ...
            <div class="mtp_div">
	                    ...
	                    <f:input id="mtp_textbox" cssStyle="width:200px;" class="easyui-textbox" path="x_MONTHLY_TARGET_POPULATION"/>
	            </div>
            </td>
            </tr>
            </table>
        ...
        <!-- form button  -->
         <div id="form_buttons">
            <a href="javascript:void(0)" id="saveBtn" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveLgaStore()" style="width:90px">Save</a>
...
function doSearch() {
	if($('#storetype_combobox').combobox('getValue')==''){
		...
		var storeNameId=$('#storename_combobox').combobox('getValue');
		 var searchUrl="search_lga_store_list?storeTypeId="+storeTypeId+"&storeNameId="+storeNameId
				 +"&storeTypeName="+storeTypeName;
		$('#lgaSoreListTable').datagrid('reload',searchUrl);
	}	 
}
...
function editLgaStore(buttonId){
	...
	submitType="edit";
	var row = $('#lgaSoreListTable').datagrid('getSelected');
    ...
    if (row){
    	if(buttonId=='editBtn'){
        $('#form_dialog').dialog('open').dialog('center').dialog('setTitle','Edit LGA Store');
    	}else{
    		  $('#form_dialog').dialog('open').dialog('center').dialog('setTitle','OverView Of LGA Store');
    	}
        ...
        $('#default_ord_store_combobox_form').combobox('setValue',row.DEFAULT_ORDERING_WAREHOUSE_ID);
       	$('#default_ord_store_combobox_form').combobox('setValue',row.DEFAULT_ORDERING_WAREHOUSE_CODE);
        $('#default_ord_store_combobox_form_field').val(row.DEFAULT_ORDERING_WAREHOUSE_ID);
        $('#default_ord_store_combobox_form_field').text(row.DEFAULT_ORDERING_WAREHOUSE_CODE);
...
function refreshLgaStoreList(){
	...
	$('#lgaSoreListTable').datagrid('reload','getlgastorelist');
}
...
function handleHistory(){
	 ...
	 if(row==null){
		 ...
		 ajaxPostRequestSync("get_LgaStore_history", {WAREHOUSE_ID: row.WAREHOUSE_ID},
				 function(response) {
	...
	$('#store_type_combobox_form').combobox({
		...
		onSelect: function(storeType) {
			...
			$('#default_ord_store_combobox_form').combobox({
				url : 'get_storename_acco_storetype_list_for_form',
				valueField : 'value',
				textField : 'label',
				queryParams:{storeTypeLabel:storeType.label}
			});
		},
...
<script type="text/javascript">
	$('#lgaSoreListTable').datagrid({
		...
		columns : [ [ {
			...
			field : 'DEFAULT_ORDERING_WAREHOUSE_CODE',
...
$('#storetype_combobox').combobox({
	...
	onSelect: function(storeTypeId) {
		...
		$('#storename_combobox').combobox({
			...
			valueField : 'value',
			textField : 'label',
			queryParams:{storeTypeId:storeTypeId.value}
		});
	}
});
//for pageination
